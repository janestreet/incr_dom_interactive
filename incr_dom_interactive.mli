...
(** ['a Interactive.t] is a monad. Within this monad, your program can receive input from
    ...
    The meaning of the ['a] parameter is that ['a Interactive.t] allows the user to
    provide your program a value of type ['a].
    ...
    Since [Interactive.t] is a monad, you can inspect the user's input and decide
    afterwards what the rest of the [Interactive.t] should be.

    For example, this defines a form which only allows the user to submit if they have
    entered at least 10 characters:
    ...
    Virtual_dom nodes with callbacks that convert user input into Incr_dom actions and
    then into Virtual_dom events using the [inject] function. For instance:

    {[
      Node.input [ Attr.on_input (fun _ev text -> inject (Action.Submit text)) ] []
    ]}

    [Interactive] works in the same way. (In fact, this is how it is implemented.) To
    render an ['a Interactive.t], you must provide functions for converting values of ['a]
    into actions and actions into Virtual_dom events. These functions are used in the
    callbacks of the Virtual_dom nodes returned by the [render] function. Then, each time
    the underlying value of the ['a Interactive.t] changes as a result of a user action
    (entering text in a text field, checking a checkbox, selecting from a drop-down menu,
    etc.), this results in an event created from the updated ['a] value.
    ...
    {[
      ...
      if is_checked then text ~init:"Foo" () else text ~init:"Bar" ()
    ]}

    Whenever the checkbox's value is changed, this recreates the text field. So if the
    user modifies the value of the checkbox, the text field's value is lost. Instead,
    ...
    {[
      ...
      let%bind_open is_checked = checkbox () in
      if is_checked then checked_text else unchecked_text
    ]}

    This code is better because if the user modifies "Foo", then checks the box and
    unchecks it again, their input will be saved. *)
type 'a t
...
module Primitives : sig
  (** [id] lets you specify a value for the "id" attribute of the DOM node. It doesn't
      affect the behavior of this library (unless you accidentally create two nodes with
      the same id), but it might be useful if you want to refer to the node elsewhere. *)
  ...
  (** [options] is a list of tuples (label, value). [label] is what will be displayed to
      the user in the dropdown. [value] is what will be produced by the [Interactive.t]
      when that option is selected by the user.

      [init] is the 0-based index of the option which is initially selected. It defaults
      to 0.
  ...
  (** [dropdown_with_blank_exn] is a wrapper around [dropdown_exn] which adds a blank
      ...
      If you don't provide a value for [init], the initial value for the dropdown will be
      the blank option which produces [None].
  ...
  (** [create] allows you to create your own primitives. [init] is the initial value of
      the [Interactive.t]. [inject] is how the users of your primitive tell it what to do
      with new values. In your event handler, you should call it and supply the new value
      of the primitive.

      You should use [value] to determine the value of your primitive. For example, if
      your primitive is a checkbox, you should do this:
      {[
      ...
      You should not use [init] to determine the value. [value] will initially be the same
      as [init], but it might be different if the field is modified, then stops being
      rendered, then is rendered again. In this case, we want the user's input to be
      preserved. But if you use [init] to fill in the initial value, the user's input will
...
(** You have to schedule an action whenever the state changes in order for the view to
    ...
    If you don't want to take any action, you should define an action which has no effect
    on your model. *)
val render
  :  'a t
  -> on_input:('a -> 'action)
...
(** [map_nodes] can be used to change the presentation of the [Interactive.t]. For
    example, the following takes an [Interactive.t] and produces a Node that has a red
